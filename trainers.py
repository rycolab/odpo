...
class BasicTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str,
        ...
        if self.config.datasets[0] != 'tldr':
            self.test_batches = self.eval_batches
        else:
            self.test_iterator = get_batch_iterator(**data_iterator_kwargs, split='eval',
                                                    n_examples=config.n_eval_examples,
                                                    batch_size=config.eval_batch_size,
                                                    silent=rank != 0, cache_dir=get_local_dir(
                    config.local_dirs), loss=config.loss.name)
    ...
    def get_common_metrics(self, metrics, losses, train_test, chosen_rewards,
                           rejected_rewards):
        reward_accuracies = (chosen_rewards > rejected_rewards).float()

        chosen_rewards = all_gather_if_needed(chosen_rewards, self.rank, self.world_size)
        rejected_rewards = all_gather_if_needed(rejected_rewards, self.rank, self.world_size)
        reward_accuracies = all_gather_if_needed(reward_accuracies, self.rank,
                                                 self.world_size)
    ...
    def get_batch_metrics_dpo(self, metrics, train_test, policy_chosen_logps,
                              policy_rejected_logps, reference_chosen_logps,
                              reference_rejected_logps, loss_kwargs, chosen_rewards=None,
                              rejected_rewards=None):
        losses, chosen_rewards, rejected_rewards = preference_loss(
            policy_chosen_logps, policy_rejected_logps, reference_chosen_logps,
            reference_rejected_logps, **loss_kwargs, chosen_rewards=chosen_rewards,
            rejected_rewards=rejected_rewards)

        policy_rejected_logps = all_gather_if_needed(policy_rejected_logps.detach(),
                                                     self.rank, self.world_size)
        metrics[f'logps_{train_test}/rejected'] = policy_rejected_logps.cpu().numpy().tolist()

        policy_chosen_logps = all_gather_if_needed(policy_chosen_logps.detach(), self.rank,
                                                   self.world_size)
        metrics[f'logps_{train_test}/chosen'] = policy_chosen_logps.cpu().numpy().tolist()
        return self.get_common_metrics(metrics, losses, train_test, chosen_rewards,
                                       rejected_rewards)

    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]],
                          loss_config: DictConfig, train=True, thresholds=None):
        """Compute the SFT or DPO loss and other metrics for the given batch of inputs."""

        metrics = {}
        train_test = 'train' if train else 'eval'

        if loss_config.name in {'dpo', 'ipo', 'odpo'}:
            policy_chosen_logps, policy_rejected_logps = self.concatenated_forward(
                self.policy, batch, loss_config.name)
            with torch.no_grad():
                reference_chosen_logps, reference_rejected_logps = self.concatenated_forward(
                    self.reference_model, batch, loss_config.name)
            ...
            if loss_config.name == 'dpo':
                loss_kwargs = {'beta': loss_config.beta,
                               ...
                               'offset': False}
                losses, metrics = self.get_batch_metrics_dpo(metrics, train_test,
                                                             policy_chosen_logps,
                ...
                loss_kwargs = {'beta': loss_config.beta, 'ipo': True, 'offset': False}
            elif loss_config.name == 'odpo':
                loss_kwargs = {'beta': loss_config.beta, 'offset': True,
                               'alpha': loss_config.alpha, 'ratio': loss_config.ratio}
                chosen_rewards, rejected_rewards = batch['chosen_reward'], batch[
                    'rejected_reward']
                losses, metrics = self.get_batch_metrics_dpo(metrics, train_test,
                                                             policy_chosen_logps,
                                                             policy_rejected_logps,
            ...
            policy_chosen_logits = self.policy(batch['chosen_input_ids'],
                                               attention_mask=batch[
                                                   'chosen_attention_mask']).logits.to(
                torch.float32)
            policy_chosen_logps = _get_batch_logps(policy_chosen_logits,
                                                   batch['chosen_labels'],
                                                   average_log_prob=False)
    ...
    def calc_kl(self, batch):
        with torch.no_grad():
            ...
            policy_logps = self.policy(batch, attention_mask=attention_mask).logits.to(
                torch.float32)
            policy_logps = _get_batch_logps(policy_logps, batch, average_log_prob=False,
            ...
            reference_logps = self.reference_model(batch,
                ...
                torch.float32)
            reference_logps = _get_batch_logps(reference_logps, batch, average_log_prob=False,
                                               pad_id=self.tokenizer.pad_token_id)
    ...
    def train(self):
        ...
        for batch in self.train_iterator:
            ...
            if self.example_counter % self.config.eval_every == 0 and (
                ...
                for eval_batch in (tqdm.tqdm(self.eval_batches,
                    ...
                    local_eval_batch = slice_and_move_batch_for_device(eval_batch, self.rank,
                                                                       ...
                                                                       self.rank)
                    with torch.no_grad():
                        _, eval_metrics = self.get_batch_metrics(local_eval_batch,
                                                                 self.config.loss,
                                                                 train=False)
                ...
                if self.config.sample_during_eval:
                    ...
                    for eval_batch in (tqdm.tqdm(sample_batches,
                        ...
                        local_eval_batch = slice_and_move_batch_for_device(eval_batch,
                                                                           self.rank,
                                                                           self.world_size,
                                                                           self.rank)
                        policy_samples, reference_samples, _, _ = self.get_batch_samples(
                            local_eval_batch)
            ...
            for microbatch_idx in range(self.config.gradient_accumulation_steps):
                ...
                loss, metrics = self.get_batch_metrics(local_microbatch, self.config.loss,
                                                       train=True)
