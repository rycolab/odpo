...
OmegaConf.register_new_resolver("get_local_run_dir",
                                lambda exp_name, local_dirs: get_local_run_dir(exp_name,
                                                                               local_dirs))
...
def worker_main(rank: int, world_size: int, config: DictConfig, policy: nn.Module,
    ...
    if rank == 0 and config.wandb.enabled:
        logging.warning("wandb is going to be initialized")
        ...
        wandb.init(
            ...
            name=f'{config.exp_name}_{config.loss.name}_{config.loss.beta}',
        )
        logging.warning("wandb is initialized")
    ...
    if config.function == 'train':
        trainer.train()
        trainer.save()
        trainer.run_evals(example_counter=trainer.example_counter)
    else:
        logging.warning("is going to sample and write function")
        trainer.run_evals()
